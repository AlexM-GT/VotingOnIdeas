...
3. Phase 1: Domain model definition. Define the initial aggregates and relationships for User, Idea, and IdeaRating. Use a one-to-many relationship from User to Ideas and from Idea to IdeaRatings. Enforce one rating per user per idea so the PUT rating endpoint behaves as an upsert. Capture owner-only authorization as an application rule for update and delete operations. Keep the MVP idea model small: title, description, owner, timestamps, and aggregated rating data.
