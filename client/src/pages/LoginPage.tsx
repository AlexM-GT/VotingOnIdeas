...
export function LoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(e?.detail ?? e?.title ?? 'Sign in failed');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="flex min-h-[calc(100vh-65px)] items-center justify-center bg-[#FAFAFA] px-4">
      <div className="w-full max-w-sm rounded-lg border border-[#D9D9D9] bg-white p-8">
        <h1 className="mb-6 text-xl font-semibold text-[#1E1E1E]">Sign in</h1>
        ...
        <p className="mt-5 text-center text-sm text-[#888]">
          No account?{' '}
          <Link to="/register" className="font-medium text-[#1E1E1E] underline underline-offset-2">
            Register
          </Link>
        </p>
      </div>
